...
void readInSolution(std::vector<img> &images, std::string xmlfilename, DatabaseFormat databaseFormat)
{
	//xml-labels
	if (databaseFormat==ICDAR2003 || databaseFormat==COBREADTEXT)
	{
		std::string label_icdar2003[] = { "file=", "tag="};
		...
		if (databaseFormat == ICDAR2003)
			...
			label = label_icdar2003;//label_cobreadtext;
		std::ifstream imgxml;
		imgxml.open(xmlfilename.c_str());
		if (imgxml.is_open())
		{
			...
			while (!imgxml.eof())
			{
				...
				// label[0] = "file="  => new image is described in xmlfile, copy filename
				if (output.find(label[0]) != std::string::npos)
				{
					filename = output.substr(output.find(label[0]) + label[0].length() + 1, output.length() - output.find(label[0]) - label[0].length() - 2);
					img i(filename);
					images.push_back(i);
				}

				// label[1]: "tag=" in xmlfile
				if (databaseFormat==ICDAR2003 && output.find(label[1]) != std::string::npos)
				{
					// texts may contain spaces!
					if ((output[output.length() - 1] != '"'))
					{
						text.append(output.substr(output.find(label[1]) + label[1].length() + 1, output.length() - output.find(label[1]) - label[1].length() - 1));
						text.append(" ");
						imgxml >> output;
						...
						text.append(output.substr(output.find(label[1]) + label[1].length() + 1, output.length() - output.find(label[1]) - label[1].length() - 2));
				...
				{
					// analyse image for letters
					std::string imgpath = xmlfilename.substr(0, xmlfilename.find_last_of("/") + 1);
					imgpath.append(images[images.size()-1].img_name);

					cv::Mat image = cv::imread(imgpath);
					cv::Mat gray_image;
					cv::cvtColor(image, gray_image, CV_RGB2GRAY);
					cv::Mat gray_image_segmented;
					//cv::threshold(gray_image, gray_image_segmented, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
					cv::adaptiveThreshold(gray_image, gray_image_segmented, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 2*cvRound(25.*gray_image.cols/200.0)+1, 0);
					cv::imshow("gray", gray_image);
					cv::imshow("otsu", gray_image_segmented);
					cv::waitKey();
...
//					angle = 0;
	...
	if (true)
	{
		...
		{
			std::cout << images[i].img_name << ": " << images[i].correctTexts[0] << std::endl;
...
//	}
	...
	if (argc < 2)
	{
		ROS_ERROR( "not enought input: letter_evaluation <img_list_name.xml>");
		return 1;
	}
